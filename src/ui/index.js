...
const COLOR_LIGHT = 0x8b6e67;
const COLOR_DARK = 0x260e04;
const COLOR_RED = 0xff0000;
const COLOR_GREEN = 0x00ff00;
const COLOR_BTN = 0x33a464;
...
export const createStatusBox = (scene, Audio, LOCATION) => {
  ...
  buttons.on("button.over", (button) => {
    background.setFillStyle(COLOR_BTN, 1.0);
  });

  buttons.on("button.out", (button) => {
    background.setFillStyle(COLOR_LIGHT, 1.0);
  });
};
...
export const createBpmSlider = () => {
  const slider = new Nexus.Slider("#bpm", {
    size: [120, 20],
    min: 30,
    max: 160,
    value: 60,
