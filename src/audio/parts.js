...
const initInstrumentParts = (weatherData) => {
  const {
    bassSampler,
    bassSynth,
    drumPlayers,
    ...
    shakeSampler,
    fmSampler,
  } = Sources.initInstruments(weatherData);
...
export const startAllInstrumentParts = (weatherData) => {
  const { drumPart, bassPart, leadPart, oceanPart, metalPart, fmPart } =
    initInstrumentParts(weatherData);
...
export const updateInstrumentPartss = ({ leadNotes, bassNotes, metalNotes }) => {
  if (leadNotes) {
    leadPart.clear();
